// Retrieved from https://docs.google.com/spreadsheets/d/e/2PACX-1vSt805o4NORCYQzL4hKByR17grc5tbcMfiw1C3vP3sKFDUOasRny-WNNMYafs__fCAqznLZAgFKrJiZ/pubhtml?urp=gmail_link
// Procedure:
// 1. Copy HTML
// 2. Paste into https://www.convertcsv.com/html-table-to-csv.htm
// 3. Remove extraneous columns and rows
...
2007,H,N/A,N/A,N/A,N/A,Open`;
const table = data
  .trim()
  .split("\n")
  .slice(1)
  .map((line) => {
    const [year, index, meanStd, median, max, medianSolvePct, round] =
      line.split(",");
    const [mean, std] = meanStd!.split(" ± ").map((x) => parseFloat(x));
    return {
      ...
      max: parseFloat(max!),
      medianSolvePct: parseFloat(medianSolvePct!.replace("%", "")) / 100,
      isOpen: round === "Open",
    };
  });
const tableByProb = new Map<string, (typeof table)[0]>();
for (const row of table) tableByProb.set(`${row.year}${row.index}`, row);
