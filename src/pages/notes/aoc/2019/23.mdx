...
I have to make some modifications to my `Program` class (which I haven't changed since [day 17](./17.mdx)'s addition of string I/O). I added a field called `default_input`, which is returned when the input queue is empty (previously it threw an error). To keep throwing errors by default, I default it to `0xDEAD`, a sentinel value. Now the code for day 23 can set it to `-1` when initializing each program.
