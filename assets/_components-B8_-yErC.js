import{j as t,L as N}from"./index-CndIh6zx.js";import{c as A}from"./index-Ciqn2WuZ.js";const c=`Year,Index,Mean ± Stdev,Median,Max,Median Solve %,Round
...
2007,H,N/A,N/A,N/A,N/A,Open`,d=c.trim().split(`
`).slice(1).map(n=>{const[a,i,o,l,I,p,v]=n.split(","),[O,r]=o.split(" ± ").map(s=>parseFloat(s));return{year:parseInt(a,10),index:i,mean:O,std:r,median:parseFloat(l),max:parseFloat(I),medianSolvePct:parseFloat(p.replace("%",""))/100,isOpen:v==="Open"}}),e=new Map;for(const n of d)e.set(`${n.year}${n.index}`,n);const m="_paginator_jay3o_1",P={paginator:m},F=A.scale(["#ca393e","#3eca39"]).domain([0,1]);function B({prob:n}){const a=e.get(n);if(!a||Number.isNaN(a.medianSolvePct))return"N/A";const i=F(a.medianSolvePct).hex();return t.jsxs("span",{style:{...a.isOpen?{backgroundColor:i,color:A(i).luminance()<.5?"white":"black"}:{border:`2px solid ${i}`,color:i},padding:"0.1em 0.4em",borderRadius:"var(--border-radius-small)",fontWeight:"bold",fontVariantNumeric:"tabular-nums"},title:`Median Solve Percentage: ${(a.medianSolvePct*100).toFixed(2)}%`,children:[(a.medianSolvePct*100).toFixed(2),"%"]})}function C({year:n}){return t.jsxs("nav",{className:P.paginator,children:[n===2022?t.jsx("span",{className:"phantom",children:"← Previous year"}):t.jsx(N,{href:`/notes/naclo/${n-1}/`,children:"← Previous year"})," ",t.jsx(N,{href:"/notes/naclo/",children:"Back to NACLO index"})," ",n===2025?t.jsx("span",{className:"phantom",children:"→ Next year"}):t.jsx(N,{href:`/notes/naclo/${n+1}/`,children:"→ Next year"})]})}export{B as D,C as Y};
